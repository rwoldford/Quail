...
(eval-when (:compile-toplevel :load-toplevel :execute) (export  '(*default-menu-font* *menu-held-wait* default-sub-item-fn
...
(defun default-sub-item-fn (menu item)
  (declare (ignore menu))
  (if (and (typep item 'sequence) (eq (fourth item) :sub-items))
    (fifth item)))
...
(defun default-when-selected-fn (item menu mouse-button)
  ...
  (if (and (typep item 'sequence) (>= (length item) 2))
...
(defun default-when-held-fn (item menu mouse-button)
  (declare (ignore menu mouse-button))
  (let ((message "This item will be selected when the button is released."))
    (if (and (typep item 'sequence) (third item))
      (setq message (third item)))
    (print message)))
