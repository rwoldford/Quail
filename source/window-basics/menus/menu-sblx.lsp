...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;++ gwb
;++ mcclim's approach to menus is so different from acl/mcl that I am temporarily
;++ ignoring the contents of this file
;++ 09SEP2020
...
(defclass wb-menu-item (wb-menu-mixin) ;(cg::menu-item)
...
(defmethod when-held-fn-of ((thing wb-menu-mixin)) ;cg::menu-item))
   (cdr
    (assoc 'when-held-fn
      (second
       (member 'wb-menu-fns
         (symbol-plist thing) ;(cg::plist thing)
         )))))
...
(defmethod (setf when-selected-fn-of) (new-value (thing wb-menu-mixin)) ;cg::menu-item))
   (setf (cdr
...
(defmethod (setf sub-item-fn-of) (new-value (thing wb-menu-mixin)) ;cg::menu-item ))
   (setf (cdr
          (assoc 'sub-item-fn
            (second
             (member 'wb-menu-fns
...
(defmethod (setf when-unheld-fn-of) (new-value (thing wb-menu-mixin)) ;cg::menu-item))
   (setf (cdr
...
(defun set-menu-fns (menu &key (selected nil selected?) (held nil held?) 
  ...
  (declare (ignorable menu selected selected? held held? unheld unheld? 
                      sub-item sub-item? user-props user-props?)) ;10MAY2024
        (format t "~%Not implemented in mcclim yet.")
        #|
     (cond
...
(defun make-menu
      (&key items sub-item-fn 
        ...
        when-held-fn when-unheld-fn
          ...
          (menu-type :pop-up)
          &aux menu)
      (declare (ignorable items sub-item-fn when-selected-fn when-held-fn when-unheld-fn 
                          title font change-offset-fn menu-type menu)) ;10MAY2024
      (format t "~%Not implemented in mcclim yet.")
      #|
...
(defun menu (menu &key (position NIL position?))
     "Pops menu up at position in screen-coordinates.
      ...
      (declare (ignorable menu position position?)) ;10MAY2024
      (format t "~%Not implemented in mcclim yet.")
      #|
...
(defun submenus? (mnu)
   "Does mnu have submenus ?~
    ...
    (declare (ignorable mnu)) ;10MAY2024
      (format t "~%Not implemented in mcclim yet.")
      #|
...
(defun list-get-menu-item (mi-list item-string)
  (declare (ignorable  mi-list item-string)) ;10MAY2024
      (format t "~%Not implemented in mcclim yet.")
      #|
   (labels
     ((lgmi-aux (mi-list)
...
(defun get-menu-item (menu item-string)
  (declare (ignorable menu item-string)) ;10MAY2024
      (format t "~%Not implemented in mcclim yet.")
      #|
   (when menu
...
(defun check-menu-item (menu item-string &optional (check-char nil))
   "Places a check-char beside item identified by item-string ~
    ...
    (declare (ignorable menu item-string check-char)) ;10MAY2024
      (format t "~%Not implemented in mcclim yet.")
      #|
   (let ((item (get-menu-item menu item-string)))
      (if item
...
(defun set-menu-item-string (menu item-string new-string)
     "Changes the TITLE for the menu item currently ~
      ...
      (declare (ignorable menu item-string new-string)) ;10MAY2024
      (format t "~%Not implemented in mcclim yet.")
      #|
     (let ((item (get-menu-item menu item-string)))
         (if item
