...
;;;     
;;;----------------------------------------------------------------------------------
;;; Collects the forms to operate on color-canvases
;;; the specialiser of  canvas  needs to exist so this file comes
;;; after color-canvas-sblx.lsp in the asd module for canvas
...
#| next 4 definitions are in redisplay/canvas-ops-sblx.lsp
...
#| next 4 forms are in redisplay/canvas-ops-sblx.lsp
;;; see canvases below
...
#| next 3 definitions are in redisplay/canvas-ops-sblx.lsp
...
(defun canvas-width  (canvas)
  ...
  (rectangle-width (exterior-frame-rect canvas)))
...
(defun canvas-height (canvas)
  ...
  (rectangle-height (exterior-frame-rect  canvas)))
...
#| next 9 definitions are in redisplay/canvas-ops-sblx.lsp
...
(let ((ftls (frame-top-level-sheet canvas)))
     (cond
               (height
                (resize-sheet ftls width height)
                )
               ((position-p width)
                (resize-sheet ftls (h-draw::point-x width) (h-draw::point-y width)))
               (region-p width)
               (resize-sheet ftls (region-width width) (region-height width))
               (T nil)
               )))
|#
;===================================================
...
; gwb august 2021
;===================================================
#| all definitions are in redisplay/canvas-ops-sblx.lsp
(defun canvas-in-screen-coords (canvas &optional (v-kludge 30))
  ...
  (let ((basic-region
  (transform-region (sheet-delta-transformation (frame-top-level-sheet canvas)
    (graft (frame-top-level-sheet canvas)))
    (sheet-region (frame-top-level-sheet canvas)))))
    (list (bounding-rectangle-min-x basic-region)
          (bounding-rectangle-max-x basic-region)
          (- (bounding-rectangle-min-y basic-region) v-kludge)
          (bounding-rectangle-max-y basic-region))))

(defun screen-point-on-canvas (canvas h v)
  "Determines whether the screen point pt h v is in or on the boundary of canvas"
  (declare (fixnum h v h-min h-max v-min v-max) (optimize (speed 3)))
...
(defun range-covered-p (list-of-canvases h-min h-max v-min v-max)
  (declare (fixnum h-min h-max v-min v-max) (optimize (speed 3)))
