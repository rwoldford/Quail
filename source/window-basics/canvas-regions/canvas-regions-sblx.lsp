...
(defun display-rect (window)
  "Returns the rectangle in which the display is drawn inside the window (canvas) in ~
  local coordinates"
  (pane-viewport-region (get-frame-pane window 'host-pane)))

  (defun display-rect-right (display-rect)
    (bounding-rectangle-max-x display-rect))

  (defun display-rect-bottom (display-rect)
    (bounding-rectangle-max-y display-rect))
#|
(defun display-rect (window)
