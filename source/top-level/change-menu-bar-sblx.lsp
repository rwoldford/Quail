...
(eval-when (:compile-toplevel :load-toplevel :execute) (export '(com-change-menu-bar)))

;(in-package :quail)
;;; The form to change command-tables
(define-qmbar-command com-change-menu-bar ((table 'symbol))
    (with-application-frame (frame)
     (setf (slot-value frame 'climi::menu-bar) (find-command-table table))
      (note-frame-command-table-changed (frame-manager frame) frame (frame-command-table frame))))