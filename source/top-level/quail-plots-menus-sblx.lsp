...
;;; plot-menu-sblx.lsp  makes only the Plots menu through the p-command-table
...
(eval-when (:compile-toplevel :load-toplevel :execute) (export '(qp-command-table)))

;;; make quail-menubar's main menus
(make-command-table 'qp-command-table ;quailmenubar-command-table ;TQUAILMENUBAR-command-table
 		    :errorp nil
 		    :menu '(("Quail" :menu q-command-table)
 		    	("Plots" :menu p-command-table)
          ;("Canvas" :menu c-command-table)
          )
 		    )