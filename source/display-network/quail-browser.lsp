...
(defclass quail-browser
  (title-bar-mixin named-object prompt-mixin browser)
       
       ;; 
       ;; The order of inheritance is important : the mixins must be before
       ;; BROWSER in the list so that quail-BROWSER inherits methods and Class
...
(defun format-for-eval (expression)
  "Format an expression containing objects so that it can be evaluated by EVAL.  ~
   ...
   the expression to eval if they are not symbols the evaluable expression ~
   is returned."
  (mapcar #'(lambda (x)
              (cond ((symbolp x)
                     x)
...
(defmethod new-item ((self quail-browser) &optional new-item)
  (if new-item
    ($! new-item)
    (set-name (prompt-eval self "quail Expression")
              (prompt-read self "Name to be given to this quail Expression"
                           ))))

(defmethod name-this-item ((self quail-browser)
                           &optional object)
...
(defmethod left-shift-select ((self quail-browser))
  (declare (special object))
  (unread self object))


(defmethod middle-shift-select ((self quail-browser))
  (declare (special object))
  (unread self object))



(defmethod title-left-shift-select ((self quail-browser))
...
(defmethod title-middle-shift-select ((self quail-browser))
...
(defmethod get-label ((self quail-browser)
                      object)
...
(defmethod get-links ((self quail-browser)
...
(defmethod show-browser ((self quail-browser)
  ...
  ;; 
  (setf (icon-of self) #'quail-icon-fn)
  self)


(defmethod icon-title ((self quail-browser))
  ;; 
...
(defmethod unread ((self quail-browser)  &optional object)
  (if object
    (push-in-buffer (access-expression object))
    (push-in-buffer (access-expression self))))


(defmethod set-name ((self quail-browser)  &optional name)
  ;; Specialization
  ;; For quail-browseRS, we must update the slot-value TITLE when the name is
  ;; changed
  ;; 
...
(defmethod call-method-and-link ((self quail-browser) expression-to-eval)
  "Given a selector obtained by selection in a menu, this method build the ~
   ...
   Updates *quail-EXPRESSION* in order to construct the spawning expression ~
   of the result later."
  ...
  (setf *quail-expression*
        (setf expression-to-eval (list 'with-link
                                       expression-to-eval)))
  (eval expression-to-eval))


(defmethod do-selected-command ((self quail-browser) command obj)
  "Does the selected command or forwards it to the object.  ~
   The value of the command is returned."
  ;;; and the macros can be executed as well as the functions or
  ;;; methods
  ;;; 
